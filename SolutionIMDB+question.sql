...
SELECT Table_name as Name_of_table , Table_rows as totalnumber_of_rows
...
SELECT 
		...
		SUM(CASE WHEN date_published IS NULL THEN 1 ELSE 0 END) AS nulls_in_date,
		...
		SUM(CASE WHEN worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS nulls_in_worlwide_gross_income,
		SUM(CASE WHEN languages IS NULL THEN 1 ELSE 0 END) AS nulls_in_languages,
		SUM(CASE WHEN production_company IS NULL THEN 1 ELSE 0 END) AS nulls_in_production_company
...
SELECT country as Country_name, year AS Year , COUNT(id) AS Total_movies
FROM movie
...
WITH count_genre AS(
SELECT movie_id, COUNT(genre) AS movie_count
FROM genre
GROUP BY movie_id
...
INNER JOIN movie AS m
ON m.id = g.movie_id
WHERE country='USA' AND MONTH(date_published)=3 AND total_votes>1000  AND year=2017
GROUP BY genre;
...
WITH genre_count AS(
WITH top_genre AS(
SELECT genre, COUNT(title) AS movie_count, RANK() OVER(ORDER BY COUNT(title) DESC) AS genre_rank
FROM movie AS m
INNER JOIN ratings AS r 
ON r.movie_id=m.id
...
top_director AS
(
SELECT n.name AS director_name, COUNT(g.movie_id) AS movie_count, Rank() OVER(ORDER BY COUNT(g.movie_id) DESC) AS director_rank
FROM names AS n 
...
genre_count
WHERE g.genre in (genre_count.genre) AND avg_rating>8
...
FROM ratings AS r
INNER JOIN role_mapping AS rlm
ON rlm.movie_id = r.movie_id
...
AVG(Round(AVG(duration),2)) OVER(ORDER BY genre ROWS 15 PRECEDING) AS moving_avg_duration
FROM movie AS m 
INNER JOIN genre AS g 
ON m.id= g.movie_id
GROUP BY genre
...
WITH selection_of_genres as(
WITH top_3_genres AS
( 	
SELECT genre, COUNT(movie_id) AS number_of_movies,RANK() OVER(ORDER BY COUNT(title) DESC) AS genre_rank
FROM genre AS g
INNER JOIN movie AS m
ON g.movie_id = m.id
GROUP BY genre)
...
top_5 AS
(
SELECT genre, year,title AS movie_name, worlwide_gross_income,RANK() OVER(PARTITION BY year ORDER BY worlwide_gross_income DESC) AS movie_rank
FROM movie AS m 
INNER JOIN genre AS g 
ON m.id= g.movie_id
WHERE genre IN (SELECT genre FROM selection_of_genres)
)
...
SELECT name AS actress_name , SUM(total_votes) AS total_votes, COUNT(rlm.movie_id) AS movie_count, avg_rating as actress_avg_rating,
RANK() OVER(ORDER BY avg_rating DESC) AS actress_rank
FROM names AS n
INNER JOIN role_mapping AS rlm
ON n.id = rlm.name_id
INNER JOIN ratings AS r
ON r.movie_id = rlm.movie_id
INNER JOIN genre AS g
ON r.movie_id = g.movie_id
...
(
SELECT name_id AS director_id, name AS director_name, dr.movie_id, duration, avg_rating , total_votes , date_published,
